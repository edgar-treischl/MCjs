...
onMounted(async () => {
  ...
  try {
    ...
    // 2️⃣ Register only necessary components for bar + line charts
    const {
      BarController,
      LineController,
      BarElement,
    ...
    // 3️⃣ Dynamically import your library
    const { BarChart, LineChart } = await import('../../../src/index.js') // adjust relative path
